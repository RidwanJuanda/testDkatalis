...
import com.example.testdkatalis.databinding.ItemNewsTopBinding
import com.example.testdkatalis.response.model.NewsData

class NewsTopAdapter(val callback: (String) -> Unit): BaseRecyclerViewAdapter<NewsData>(){
    ...
    override fun onCreateViewHolder(
        ...
        return PopularNewsViewHolder(
            ItemNewsTopBinding.inflate(
                LayoutInflater.from(parent.context), parent, false
            )
        )
    }

    inner class PopularNewsViewHolder(val view:ItemNewsTopBinding):BaseViewHolder<NewsData>(view.root){
